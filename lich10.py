...
try:
    ...
    with st.sidebar:
        ...
        auto_adjust_option = st.checkbox("Tự động điều chỉnh công khi có người công tác", value=True, key="sidebar_auto_adjust_checkbox")
    ...
    with tab3:
        ...
        with col1:
            st.metric("Tổng nhân sự", len(all_staff), key="tab3_metric_total_staff")
        
        with col2:
            st.metric("Trưởng kiếp", len(truong_kiep), key="tab3_metric_tk_count")
        
        with col3:
            st.metric("Vận hành viên", len(van_hanh_vien), key="tab3_metric_vhv_count")
        
        with col4:
            st.metric("Ngày đào tạo", f"Ngày {training_day}", key="tab3_metric_training_day")
        ...
        if st.session_state.schedule_created and st.session_state.staff_stats:
            ...
            with col1:
                st.metric("Người cân bằng", f"{balanced_count}/{total_count}", key="tab3_metric_balanced")
            with col2:
                st.metric("Tỷ lệ cân bằng", f"{balance_percentage:.1f}%", key="tab3_metric_balance_percent")
            with col3:
                avg_diff = sum(stat['Chênh lệch tuyệt đối'] for stat in balance_stats) / total_count
                st.metric("Chênh lệch TB", f"{avg_diff:.1f}", key="tab3_metric_avg_diff")
            ...
            with col1:
                st.metric("Tổng ca trực", total_shifts, key="tab3_metric_total_shifts")
            with col2:
                st.metric("Ngày công tác", total_business, key="tab3_metric_total_business")
            with col3:
                st.metric("Nhóm kiểm tra", total_inspection, key="tab3_metric_total_inspection")
            with col4:
                st.metric("Tổng công thực", f"{total_actual}/{total_target}", key="tab3_metric_total_actual")
        else:
    ...
    st.markdown("---")
    st.caption("""
    **Hệ thống xếp lịch trực TBA 500kV - Phiên bản 7.0 - Hỗ trợ kiểm tra đường dây**  
