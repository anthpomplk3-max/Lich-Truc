...
try:
            ...
            with col_tk:
                ...
                for idx, tk in enumerate(truong_kiep):
                    with st.expander(f"**{tk}**", expanded=False):
                        ...
                        # Thêm slider cho số ca đêm mong muốn - ĐÃ SỬA TỪ 10 LÊN 17
                        night_goal = st.slider(
                            f"Số ca đêm mong muốn - {tk}",
                            min_value=0,
                            max_value=17,  # Đã thay đổi từ 10 lên 17
    ...
    with tab2:
        st.subheader("Tạo lịch trực tự động và xem lịch ngang")
        ...
        if st.session_state.schedule_created and st.session_state.staff_horizontal_schedule is not None:
            ...
            with col2:
                ...
                **QUY TẮC CỨNG:**
                ...
                - Công kiểm tra: 1 công/ngày (KT) - TK 1 công, VHV 1 công
        ...
        if st.session_state.schedule_created and st.session_state.schedule_data:
            ...
            with col2:
                ...
                if start_day <= end_day:
                    ...
                    if conflicts:
                        st.warning(f"⚠️ **CẢNH BÁO**: {emergency_staff} đã có lịch trực trong các ngày sau. Các ca trực này sẽ bị hủy và xếp lại:")
                        for conflict in conflicts:
                            st.warning(f"- {conflict}")
                        # Vẫn cho phép điều chỉnh, nhưng cảnh báo
                    else:
            ...
            with col1:
                if st.button("🔄 Điều chỉnh lịch", type="primary", use_container_width=True, key="tab4_adjust_schedule_btn"):
            ...
            with col2:
                if st.session_state.original_schedule is not None:
                    if st.button("↩️ Khôi phục lịch gốc", type="secondary", use_container_width=True, key="tab4_restore_schedule_btn"):
                        # Khôi phục lịch gốc
            ...
            # Hiển thị phần lịch bị ảnh hưởng - CHỈ HIỂN THỊ LỊCH NGANG
            st.markdown("---")
            st.subheader("📋 Lịch trực ngang sau điều chỉnh (các ô màu vàng là thay đổi)")
            
            if st.session_state.adjusted_horizontal_schedule is not None:
                # Tạo style cho lịch đã điều chỉnh
                ...
                # Hiển thị lịch ngang với style
                st.dataframe(
                    styled_schedule,
                    use_container_width=True,
                    height=min(600, 150 + len(st.session_state.adjusted_horizontal_schedule) * 35)
                )
            ...
            elif st.session_state.staff_horizontal_schedule is not None:
                # Hiển thị lịch ngang hiện tại
                st.dataframe(
                    st.session_state.staff_horizontal_schedule,
                    use_container_width=True,
                    height=min(600, 150 + len(st.session_state.staff_horizontal_schedule) * 35)
                )
            ...
            **HƯỚNG DẪN ĐIỀU CHỈNH CÔNG TÁC ĐỘT XUẤT:**
            ...
            5. Hệ thống sẽ:
               ...
               - Đảm bảo mỗi nhân viên đạt đúng 17 công (nếu không có công tác khác)
            6. Có thể khôi phục về lịch gốc bất kỳ lúc nào
            """)
            
            # Hiển thị chú thích màu sắc
            st.markdown("""
            **CHÚ THÍCH MÀU SẮC SAU ĐIỀU CHỈNH:**
            - 🟨 **Màu vàng**: Ô bị thay đổi sau khi điều chỉnh
            - 🟩 **Màu xanh lá nhạt**: Ca trực
            - 🟧 **Màu cam nhạt**: Kiểm tra đường dây
            - 🟥 **Màu đỏ nhạt**: Công tác
            - ⬜ **Màu xám**: Ngày nghỉ
            - 🟪 **Màu tím nhạt**: Đào tạo
            """)
        else:
    ...
    st.markdown("---")
    st.caption("""
    **Hệ thống xếp lịch trực TBA 500kV - Phiên bản 14.0 - LỊCH NGANG ĐẢM BẢO 17 CÔNG**  
    ...
    *Ngày đào tạo: vẫn có ca trực bình thường*  
    *Có thể đặt mục tiêu số ca đêm mong muốn (0-17)*  
