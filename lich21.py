    ...
    def generate_advanced_schedule(month, year, training_day, day_off_dict, business_trip_dict, line_inspection_groups, night_shift_goals, balance_shifts=True, allow_tk_substitute_vhv=False):
        ...
        if not has_business_trip:
            ...
            if total_required_from_shifts > total_available_shifts:
                ...
                st.error(f"❌ KHÔNG ĐỦ CA TRỰC: Cần {total_required_from_shifts} ca trực, nhưng chỉ có {total_available_shifts} ca.")
            ...
            # Nếu đủ ca, thì target_shifts của mỗi người là required_shift_credits (đã tính ở trên)
            # Không cần điều chỉnh target_shifts, vì mỗi người đã có target_shifts = required_shift_credits
