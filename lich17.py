...
try:
    ...
    with st.sidebar:
        ...
        **Quy tắc xếp lịch:**
        ...
        5. Ngày đào tạo: tất cả có mặt (tính 1 công hành chính) + vẫn có ca trực bình thường
        ...
        10. Ngày đào tạo: cho phép làm việc 24h (tham gia đào tạo + trực ca đêm)
        """)
    ...
    # Thuật toán xếp lịch nâng cao - ĐÃ SỬA ĐỂ ĐẢM BẢO 17 CÔNG VÀ NGÀY ĐÀO TẠO CÓ TRỰC
    def generate_advanced_schedule(month, year, training_day, day_off_dict, business_trip_dict, line_inspection_groups, night_shift_goals, balance_shifts=True):
        ...
        # Kiểm tra xem có đủ ca để phân bổ không
        if total_required_shifts > total_available_shifts:
            # Tính hệ số điều chỉnh
            adjustment_factor = total_available_shifts / total_required_shifts
            
            # Điều chỉnh target_shifts cho mỗi nhân viên
            for staff in all_staff:
                staff_data[staff]['target_shifts'] = max(1, int(staff_data[staff]['target_shifts'] * adjustment_factor))
        ...
        for day in working_days:
            ...
            if available_tk_day and available_vhv_day:
                ...
                if selected_tk and selected_vhv:
                    ...
                    selected_tk = select_staff_for_shift_emergency_17(
                        available_tk_day, staff_data, day, 'day', 'TK', is_training_day
                    )
                    selected_vhv = select_staff_for_shift_emergency_17(
                        available_vhv_day, staff_data, day, 'day', 'VHV', is_training_day
                    )
                    if selected_tk and selected_vhv:
                        update_staff_data_17(staff_data, selected_tk, day, 'day')
                        update_staff_data_17(staff_data, selected_vhv, day, 'day')
                        ...
                        if is_training_day:
                            ghi_chu = 'Đào tạo + Trực ca ngày (Khẩn cấp)'
            ...
            if available_tk_night and available_vhv_night:
                ...
                if selected_tk_night and selected_vhv_night:
                    # Cập nhật thông tin
                    update_staff_data_17(staff_data, selected_tk_night, day, 'night')
                    update_staff_data_17(staff_data, selected_vhv_night, day, 'night')
                    ...
                    selected_tk_night = select_staff_for_shift_emergency_17(
                        available_tk_night, staff_data, day, 'night', 'TK', is_training_day
                    )
                    selected_vhv_night = select_staff_for_shift_emergency_17(
                        available_vhv_night, staff_data, day, 'night', 'VHV', is_training_day
                    )
                    if selected_tk_night and selected_vhv_night:
                        update_staff_data_17(staff_data, selected_tk_night, day, 'night')
                        update_staff_data_17(staff_data, selected_vhv_night, day, 'night')
                        ...
                        if is_training_day:
                            ghi_chu = 'Đào tạo + Trực ca đêm (Khẩn cấp)'
    ...
    def update_staff_data_17(staff_data, staff, day, shift_type):
        """Cập nhật thông tin nhân viên sau khi phân công - Phiên bản đảm bảo 17 công"""
        if shift_type == 'day':
    ...
    def select_staff_for_shift_17(available_staff, staff_data, day, shift_type, role, balance_shifts=True, last_days_mode=False, is_training_day=False):
        """Chọn nhân viên phù hợp cho ca làm việc - ĐẢM BẢO 17 CÔNG"""
        ...
        if not filtered_staff:
            ...
            for staff in available_staff:
                ...
                if remaining_to_17 <= 0 and data['total_shifts'] >= data['target_shifts'] + 2:
                    continue
        ...
        filtered_staff.sort(key=lambda x: (
            ...
            calculate_shift_balance_score_17(staff_data[x], shift_type, balance_shifts),
    ...
    def select_staff_for_shift_emergency_17(available_staff, staff_data, day, shift_type, role, is_training_day=False):
        """Chọn nhân viên trong trường hợp khẩn cấp - Phiên bản đảm bảo 17 công"""
        if not available_staff:
            return None
        
        # Trong trường hợp khẩn cấp, chỉ giữ điều kiện không làm 24h liên tục (trừ ngày đào tạo)
        filtered_staff = []
        for staff in available_staff:
            data = staff_data[staff]
            
            # Tính số công còn thiếu để đạt 17
            remaining_to_17 = 17 - data['current_total_credits']
            
            # Chỉ kiểm tra không làm 24h liên tục (trừ ngày đào tạo)
            if shift_type == 'night' and not is_training_day and data['last_shift'] == 'day' and data['last_shift_day'] == day:
                continue
        ...
        # Ưu tiên người còn thiếu nhiều công nhất để đạt 17
        filtered_staff.sort(key=lambda x: (
            17 - staff_data[x]['current_total_credits'],  # Người còn thiếu nhiều công nhất
            staff_data[x]['total_shifts'],  # Sau đó ưu tiên người ít ca
            random.random()
        ))
    ...
    def calculate_shift_balance_score_17(staff_data, shift_type, balance_shifts):
        """Tính điểm cân bằng ca ngày/đêm - Phiên bản đảm bảo 17 công"""
            ...
            if st.button("🎯 Tạo lịch trực tự động", type="primary", use_container_width=True, key="tab2_generate_schedule_btn"):
                with st.spinner("Đang tạo lịch trực với cân bằng ca và kiểm tra đường dây..."):
                    ...
                    schedule, staff_data = generate_advanced_schedule(
                        ...
                        business_trip_dict, line_inspection_groups, night_shift_goals, balance_shifts_value
                    )
                    ...
                    if under_17:
                        ...
                        st.info("Nguyên nhân có thể do: quá nhiều ngày nghỉ/công tác, thiếu ca trực trong tháng.")
    ...
    with tab3:
        ...
        if st.session_state.schedule_created and st.session_state.staff_stats:
            ...
            # Cảnh báo nếu có người chưa đạt 17 công
            if under_17_count > 0:
                st.error(f"⚠️ **CẢNH BÁO:** Có {under_17_count} người chưa đạt 17 công/tháng!")
                st.info("""
                **Nguyên nhân có thể:**
                1. Quá nhiều ngày nghỉ/công tác/kiểm tra đường dây
                2. Số ca trực trong tháng không đủ phân bổ
                3. Xung đột lịch trình (nghỉ/công tác trùng nhiều ngày)
                
                **Giải pháp:**
                - Giảm số ngày nghỉ/công tác
                - Điều chỉnh ngày kiểm tra đường dây
                - Kiểm tra lại ngày đào tạo
                """)
    ...
    with tab4:
        ...
        if st.session_state.schedule_created and st.session_state.staff_horizontal_schedule is not None:
            ...
            with col2:
                st.markdown("""
                **Quy tắc công (BẮT BUỘC):**
                - **Tổng công tối thiểu: 17 công/tháng**
    ...
    st.markdown("---")
    st.caption("""
    **Hệ thống xếp lịch trực TBA 500kV - Phiên bản 10.0 - NGÀY ĐÀO TẠO CÓ TRỰC & MỤC TIÊU CA ĐÊM**  
    *Mỗi người: 17 công/tháng = 1 công đào tạo + công kiểm tra + công công tác + công trực ca*  
    *Ngày đào tạo: vẫn có ca trực bình thường, cho phép làm 24h*  
    **Có thể đặt mục tiêu số ca đêm mong muốn cho mỗi người**  
    *Ưu tiên đạt mục tiêu ca đêm khi xếp lịch*  
